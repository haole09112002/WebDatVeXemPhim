...
public class LichChieuDAO implements INewDAO<LichChieu> {
	...
	public int add(LichChieu t) {
		...
		try {
			...
			statement.setInt(2, t.getIdPhong());
			statement.setDate(3, t.getNgayChieu());
			statement.setInt(4, t.getIdGioChieu());
			statement.setDouble(5, t.getGiaVe());
	...
	public List<LichChieu> getLichChieuByNgayChieuIdPhong(Date date, int idP) {
		...
		List<LichChieu> results = new ArrayList<>();
		String sql = "Select * from LichChieu where ngayChieu = ? and idPhong = ?";
		PreparedStatement statement = null;
		try {
			statement = DBHelper.getInstance().getConnection().prepareStatement(sql);
			statement.setDate(1, date);
